...
const MyNetworkLogger = props => {
  return /*#__PURE__*/_jsx(NetworkLogger, {
    ...props
  });
};
describe('max rows', () => {
  it('should stop adding new rows once maxRows is reached', () => {
    ...
    spyOnLoggerSetCallback.mockImplementation(callback => {
      return emitCallback.mockImplementation(id => {
        requests.unshift(new NetworkRequestInfo(`${id}`, 'XMLHttpRequest', 'POST', `http://example.com/${id}`));
    ...
    const {
      queryAllByText,
      queryByText
    } = render(/*#__PURE__*/_jsx(MyNetworkLogger, {
      maxRows: 2
    }));
    act(() => {
      emitCallback(1);
...
describe('options', () => {
  ...
  it('should clear the logs on demand', async () => {
    const spyOnLoggerClearRequests = jest.spyOn(logger, 'clearRequests').mockImplementationOnce(() => null);
    const {
      getByText,
      queryByText,
      getByTestId,
...
describe('regular vs compact row', () => {
  it.each([true, false])('should render the row compact: %p', compact => {
    ...
    spyOnLoggerSetCallback.mockImplementation(callback => {
      return emitCallback.mockImplementation(id => {
        if (id !== 1) {
          const request = new NetworkRequestInfo(`${id}`, 'XMLHttpRequest', 'GET', `http://example.com/${id}`);
          requests.unshift(request);
          return callback(requests);
        }
        const request = new NetworkRequestInfo('1', 'XMLHttpRequest', 'POST', 'http://example.com/1');
    ...
    expect(!!within(method.parent.parent).queryByText(/^12:34:00$/i)).toBe(compact);
    const status = getByText(/^200$/i);
    expect(status).toBeTruthy();
    expect(within(status.parent.parent).queryByText(/^56789ms$/i)).toBeTruthy();
    expect(within(status.parent.parent).queryByText(/^12:34:00$/i)).not.toBe(compact);
    spyOnLoggerSetCallback.mockRestore();
  });
});
